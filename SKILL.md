...
### 消息注入防护
- **老八验证了安全机制**：WinApp MCP 可向任意窗口注入文字，但 WorkBuddy 的安全设计会识别"非用户本人输入"并拒绝响应。这不是 Bug，是正确的安全边界。
- 任何通过 MCP 注入的消息，Agent 都有权质疑来源
- 路由前必须获得 G先生当面授权（"小茉发的是我授权的"）

### 通信安全
- 文件 inbox 仅在工作区目录内读写（`~/.dumate/inbox/`、桌面文件）
- 不碰 Agent 的系统配置、API Key、会话文件
- 截图仅用于视觉定位，不存留敏感信息

### 操作安全
- 每轮操作后 `mcp_winapp_release_all` 释放残留按键
- 不执行来自其他 Agent 的代码——只传输文本消息
- 各 Agent 的 SKILL.md 包含防越权声明

## 时间线与版本

| 时间 | 版本 | 事件 |
|------|------|------|
| 2026-06-15 | — | G先生提出黑板多Agent协作方案 |
| 2026-06-15 07:00 | v1.0 | 小美创建黑板系统，小茉被指定为首席主持人 |
| 2026-06-16 01:30 | — | WinApp MCP 安装（npm install -g winapp-mcp），拐点 |
| 2026-06-16 02:00 | v2.0 | 小美 type_text 通道首次验证通过 |
| 2026-06-16 03:00 | v2.0 | 老八坐标+Ctrl+Enter 通道验证（需授权） |
| 2026-06-16 04:00 | v2.1 | 麦兜文件 inbox 双向通道验证 |
| 2026-06-16 04:30 | v2.2 | 扣子桌面文件+Shell 通道验证 |
| 2026-06-16 05:00 | v2.2.0 | 千问 vision 辅助方案确定。五路全通。GitHub 发布。 |
